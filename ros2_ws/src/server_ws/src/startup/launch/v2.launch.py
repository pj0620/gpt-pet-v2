...
def generate_launch_description():
    ...
    slam_toolbox_node = Node(
        ...
        parameters=[
            os.path.join(pkg, 'config', 'slam_toolbox_parameters.yaml'),
            {
              ...
              'minimum_travel_distance': 0.00001,
            },
        ],
    )
    nodes.append(slam_toolbox_node)

    # Always start with a fresh map: reset slam_toolbox map on launch
    clear_slam_map = TimerAction(
        period=3.0,
        actions=[
            ExecuteProcess(
                cmd=[
    ...
    # Add static transform from odom to laser (lidar frame 0.2m above odom,
